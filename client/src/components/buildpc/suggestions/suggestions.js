...
export const Suggestions = (props) => {
  ...
  function getSuggestionItems() {
    if (components && attributes.length > 0) {
      ...
      if (cpuType && attributes[count].type === "motherboard") {
        ...
        if (cpuType.integratedGraphics === false) {
          currentType = currentType.filter(
            (c) => c._id !== "60aa88cef4bbec924111bf02"
          );
        }
      }
  ...
  return (
    <div className="buildpc">
      <h1>
        {attributes.length > 0
          ? "Select " + attributes[count].title
          : "Loading..."}
      </h1>
      <div className="suggestions-container">{getSuggestionItems()}</div>
